...
COPY rhdh-product-docs-plaintext ./rhdh-product-docs-plaintext
...
COPY scripts/common_embeddings.py .
RUN export LD_LIBRARY_PATH=/usr/local/cuda-12.6/compat:$LD_LIBRARY_PATH; \
    set -e && for RHDH_VERSION in $(ls -1 rhdh-product-docs-plaintext); do \
        pdm run python generate_embeddings.py -f rhdh-product-docs-plaintext/${RHDH_VERSION} -md embeddings_model \
            -mn ${EMBEDDING_MODEL} -o vector_db/rhdh_product_docs/${RHDH_VERSION} \
            -i rhdh-product-docs-$(echo $RHDH_VERSION | sed 's/\./_/g') -v ${RHDH_VERSION}; \
    done

FROM registry.access.redhat.com/ubi9/ubi-minimal@sha256:c0e70387664f30cd9cf2795b547e4a9a51002c44a4a86aa9335ab030134bf392
COPY --from=lightspeed-rag-builder /workdir/vector_db/rhdh_product_docs /rag/vector_db/rhdh_product_docs
